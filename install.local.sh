...
print_next_steps() {
    ...
    echo "     cargo run -p oore-server"
    echo ""
    echo "  2. Start the web dashboard (in another terminal):"
    echo "     cd web && bun dev"
    echo ""
    echo "  3. Test with the CLI:"
    echo "     cargo run -p oore-cli -- health"
    echo ""
    echo "  4. Optional - Setup GitHub integration:"
    echo "     cargo run -p oore-cli -- github setup"
    echo ""
    echo -e "For more information, see: ${BLUE}https://oore.build/docs${NC}"
    echo ""
}
