...
Complete profile setup enrolls a managed Direct macOS runner on the control-plane Mac.
An operator can add a Runner profile on another Mac. In both placements:

- `oored` remains the control-plane authority
...
The control plane returns the registration token once, and the CLI stores it in the
...
Before a runner claims queued work, the control plane checks that the instance is
...
- Link only repositories that you trust under the Runner account.
