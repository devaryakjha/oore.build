...
Oore uses provider discovery, the authorization code flow with PKCE, and the scopes `openid` and `email`. The provider authenticates the person; Oore then creates its own session and enforces Oore roles.
...
3. Enter the **Client secret** when the provider created a confidential web client. Leave it blank only for a public client.
