...
In Oore's **OIDC** step, select **Microsoft Entra ID** under **Identity provider**, then enter:
...
Replace `<tenant-id>` with the Directory (tenant) ID and select **Configure OIDC**.
...
### Entra says that the account has no directory

A personal Microsoft account without an Entra tenant cannot register this application. Use an organization account with app registration access, or create a tenant first.
