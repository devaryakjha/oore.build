...
For the supported hosted UI path through Cloudflare, follow [Connect Cloudflare
Tunnel and Access](/operate/access/cloudflare-access). It uses Oore OIDC for
application identity. Its advanced section limits this mode to disposable
acceptance until an adapter validates the Access JWT.
