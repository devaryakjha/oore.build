...
title: 'Upgrade Oore'
status: implemented
description: 'Preserve data, remove live components, then install the next release with the same profile.'
---

In v0.1.42, `oore install` does not replace live profile components across releases.

`oore update --check` can inspect the next release. Direct `oore update` stops without changes for profile installations in v0.1.42.

If recorded values are unknown, run `oore update`. It stops before changes and prints the recorded profile values.
