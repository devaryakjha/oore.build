...
#[cfg(test)]
mod tests {
    use super::{IncidentInput, open_incident, resolve_incident};

    #[tokio::test]
    async fn incidents_deduplicate_resolve_and_notify_only_source_managers() {
        ...
        sqlx::raw_sql(
            ...
            CREATE TABLE operator_incidents (
                ...
                status TEXT NOT NULL, severity TEXT NOT NULL, reason TEXT NOT NULL,
                resource_kind TEXT NOT NULL, resource_id TEXT NOT NULL,
                resource_name TEXT NOT NULL, repair_action TEXT NOT NULL,
                repair_url TEXT NOT NULL, audience_resource TEXT NOT NULL,
                audience_action TEXT NOT NULL, first_occurrence_at INTEGER NOT NULL,
                latest_occurrence_at INTEGER NOT NULL, occurrence_count INTEGER NOT NULL,
                resolved_at INTEGER, created_at INTEGER NOT NULL, updated_at INTEGER NOT NULL
            );
            CREATE TABLE operator_incident_notifications (
                id TEXT PRIMARY KEY, incident_id TEXT NOT NULL, user_id TEXT NOT NULL,
                created_at INTEGER NOT NULL, read_at INTEGER,
                UNIQUE (incident_id, user_id)
            );
            INSERT INTO users VALUES ('owner', 'owner', 'active');
            INSERT INTO users VALUES ('admin', 'admin', 'active');
            INSERT INTO users VALUES ('developer', 'developer', 'active');
            INSERT INTO users VALUES ('disabled-admin', 'admin', 'disabled');",
        )
        .execute(&pool)
        .await
        .unwrap();
        ...
        let input = || IncidentInput {
            deduplication_key: "source-credential:gitlab",
            severity: "critical",
            reason: "rejected",
            resource_kind: "source",
            resource_id: "gitlab",
            resource_name: "GitLab",
            repair_action: "Replace access token",
            repair_url: "/settings/integrations/gitlab?tab=connection",
            audience_resource: "integrations",
            audience_action: "write",
        };
        let first_id = open_incident(&pool, input()).await.unwrap();
        let duplicate_id = open_incident(&pool, input()).await.unwrap();
        assert_eq!(first_id, duplicate_id);

        let occurrence_count: i64 =
            sqlx::query_scalar("SELECT occurrence_count FROM operator_incidents WHERE id = ?1")
                .bind(&first_id)
                .fetch_one(&pool)
                .await
                .unwrap();
        let notified_users: Vec<String> = sqlx::query_scalar(
            "SELECT user_id FROM operator_incident_notifications ORDER BY user_id",
        )
        .fetch_all(&pool)
        .await
        .unwrap();
        assert_eq!(occurrence_count, 2);
        assert_eq!(notified_users, vec!["admin", "owner"]);

        resolve_incident(&pool, "source-credential:gitlab")
            .await
            .unwrap();
        open_incident(&pool, input()).await.unwrap();
        let state: (String, i64, Option<i64>) = sqlx::query_as(
            "SELECT status, occurrence_count, resolved_at FROM operator_incidents WHERE id = ?1",
        )
        .bind(&first_id)
        .fetch_one(&pool)
        .await
        .unwrap();
        assert_eq!(state, ("open".to_string(), 3, None));
    }
}
