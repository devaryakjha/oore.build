...
banner:
  content: |
    <strong>Early Development</strong> — Architecture is still being evaluated and may change.
---
...
│                     │                    │  ┌──────────────────────────────┐   │
│  ┌───────────────┐  │      HTTPS         │  │          oored               │   │
│  │  oore (CLI)   │──┼───────────────────▶│  │       (HTTP server)          │   │
│  └───────────────┘  │                    │  │                              │   │
│                     │                    │  │  ┌────────┐  ┌────────────┐  │   │
│  ┌───────────────┐  │      HTTPS         │  │  │REST API│  │ Background │  │   │
│  │    Browser    │──┼───────────────────▶│  │  │        │  │   Worker   │  │   │
│  └───────────────┘  │                    │  │  └────────┘  └────────────┘  │   │
│                     │                    │  └──────────────────────────────┘   │
└─────────────────────┘                    │           │                         │
...
| `oore` (CLI) | Anywhere | Just an HTTP client |
...
- **Commands**: Repository, build, webhook, config management
- **Configuration**: Profile-based config in `~/.oore/config.huml`
...
<Tabs>
  ...
  <TabItem label="Production">
    ...
    ~/.oore/config.huml          # CLI configuration (per-user)
