...
Oore has two configuration systems:

1. **Server configuration** — Environment variables for `oored` (the server daemon)
2. **CLI configuration** — Profile-based config in `~/.oore/config.huml` for `oore` (the CLI client)

This page covers server configuration. For CLI configuration, see [CLI Reference - Configuration File](/docs/reference/cli/#configuration-file).
