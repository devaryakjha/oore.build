...
banner:
  content: |
    <strong>Alpha</strong> — Configuration is functional. Options may change as features are added.
---

import { Tabs, TabItem } from '@astrojs/starlight/components';

Oore is configured through environment variables for the `oored` server daemon.

---

## Server Configuration

The server (`oored`) is configured through environment variables. These can be set in a `.env` file or passed directly to the process.
...
| `DATABASE_MAX_CONNECTIONS` | No | `20` | Maximum database connection pool size |
...
| `OORE_DEMO_MODE` | No | `false` | Enable demo mode with simulated data (for UI testing) |
...
:::tip[Demo Mode]
Set `OORE_DEMO_MODE=true` for local UI development. This provides fake repositories, builds, logs, and pre-configured GitHub/GitLab integrations without requiring real OAuth setup. See the [Demo Mode Guide](/docs/guides/demo-mode/) for details.
:::
...
:::tip[Recommended: Use Dashboard Setup]
The recommended way to configure GitHub is using the web dashboard setup flow, which stores credentials encrypted in the database. No environment variables needed!

See [GitHub Integration](/docs/integrations/github/) for the complete setup guide.
:::

**Environment variables (legacy/alternative):**

These are only needed if you're manually configuring a pre-existing GitHub App instead of using the dashboard setup flow:
...
| Variable | Required | Default | Description |
|----------|----------|---------|-------------|
| `GITHUB_MAX_REPOS_PER_INSTALLATION` | No | `1000` | Maximum repositories to sync per installation (DoS protection) |
...
### With GitHub App (Dashboard Setup - Recommended)
...
Then use the web dashboard to configure the GitHub App. Credentials are stored encrypted in the database.
...
sudo chown root:wheel /etc/oore/oore.env
```
