...
banner:
  content: |
    ⚠️ <strong>Experimental</strong> — Configuration options may change without notice. For contributors only.
---
...
This page covers server configuration. For CLI configuration, see [CLI Reference → Configuration File](/reference/cli/#configuration-file).
...
:::tip[Demo Mode]
Set `OORE_DEMO_MODE=true` for local UI development. This provides fake repositories, builds, logs, and pre-configured GitHub/GitLab integrations without requiring real OAuth setup. See the [Demo Mode Guide](/guides/demo-mode/) for details.
:::
