...
## Session 8: Bug Fix - Invalid Date Crash

### Summary
Fixed a crash on the GitLab settings page caused by null dates being passed to format functions.

### The Bug

After the i18n changes in Session 5 (switching to `Intl.DateTimeFormat`), the GitLab settings page would crash with:
```
Invalid time value
```

**Root cause**: The `token_expires_at` field from the API can be `null` for credentials that don't expire. When `new Date(null)` is called, it creates an "Invalid Date" object, and `Intl.DateTimeFormat.format()` throws on invalid dates.

### The Fix

Updated all date formatting functions in `lib/format.ts` to handle null/undefined:
...
- `lib/format.ts` - Added null checks and `isValidDate` helper

### Verification

- Build: ✓ `bun run build` passes
- GitLab settings page no longer crashes with null dates

---

## Session 9: Early Development Warnings

### Summary
Added prominent experimental warnings across all user-facing surfaces (README, docs, landing page) to clearly communicate that the project is in very early development.

### What Was Added

**README.md**:
- Added GitHub `[!CAUTION]` callout at the top
- Updated Project Status section to be honest about what works vs what doesn't
- Changed "Implemented" to "Scaffolded" with note about plumbing vs core features

**Documentation (10 pages updated)**:
- `index.mdx` - Updated banner and project status
- `introduction.mdx` - Banner + `<Aside type="caution">` block
- `quickstart.mdx` - Banner + "For Contributors Only" aside
- `configuration.mdx` - Banner
- `architecture.mdx` - Banner
- `guides/service-management.mdx` - Banner
- `integrations/github.mdx` - Banner + caution aside
- `integrations/gitlab.mdx` - Banner + caution aside
- `reference/cli.mdx` - Banner
- `reference/api.mdx` - Banner

**Landing Page**:
- Added sticky warning banner at top of all pages (yellow on dark)
- Updated Status component:
  - Changed "Implemented" (green) → "Scaffolded" (yellow)
  - Changed "In Progress" → "Not Started" (red)
  - Added note: "The scaffolded items are basic plumbing"

### Warning Text Pattern

All warnings follow the same tone:
> ⚠️ **Experimental** — This project is in very early development. Most features don't work yet. Do not use for real projects.
...
## Session 10: Web Dashboard Screenshots

### Summary
Captured screenshots of the web dashboard for the README showcase section.

### Screenshots Captured

| Screenshot | Page | Contents |
|------------|------|----------|
| `dashboard.png` | `/` | Stats cards, setup progress, quick actions, recent builds |
| `repositories.png` | `/repositories` | Repository table with provider, branch, status |
| `builds.png` | `/builds` | Build history with status badges |
| `settings.png` | `/settings` | Integration configuration cards |

### Technical Details

- Resolution: 1728x1117 (MacBook Pro 16" scaled)
- Theme: Dark mode
- Tool: Playwright MCP
- Format: PNG
...
Added Screenshots section after "What is Oore?" with 2x2 table layout:
```markdown
## Screenshots
...
|---------|-------|------------|
| 1 | GitLab Automated Setup | `oore gitlab setup` command with browser flow |
| 2 | GitLab UX | Auto-detection of single instance |
| 3 | Web Dashboard | Complete Next.js dashboard with all pages |
| 4 | UI Review - Security | Removed console.error, created URL validation |
| 5 | UI Review - Accessibility | aria-labels, Intl.DateTimeFormat, color-scheme |
| 6 | UI Review - Typography | Ellipsis fixes, autocomplete attributes |
| 7 | UI Review - URLs | Applied URL validation to external links |
| 8 | Bug Fix | Fixed null date crash in format functions |
| 9 | Warnings | Added experimental warnings everywhere |
| 10 | Screenshots | Captured dashboard screenshots for README |

### Commits Today

```
7041bd5 docs: add web dashboard screenshots to README
ebbb4bf feat(landing): add experimental warning banner and update status
a37d01a chore: add Zerodha Tech badge to README and ignore ESLint cache
751ec26 docs: add prominent experimental warnings to all pages
c7996c3 docs: add early development warning to README
e8f4ddc docs: update progress log and project instructions
fb629e8 feat(api): add installation repositories endpoint for dashboard
d0c06a4 feat(web): complete dashboard implementation with UI review fixes
```
